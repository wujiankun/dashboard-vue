<template>
  <div class="container">
    <el-card>
      <div slot="header">
        <div>课程：{{ courseInfo.courseId }}</div>
        <div>阶段：{{ courseInfo.sectionId }}</div>
        <div>课时：{{ courseInfo.duration || '未知' }}</div>
      </div>
      <el-form label-width="40px">
        ...
        <el-form-item v-if="uploadStarted">
          <p>视频上传中...{{ uploadPercent }}%</p>
          <template v-if="uploadSuccess">
            <p v-if="!transCodeSuccess">视频转码中...</p>
            <p v-else>视频转码完成</p>
          </template>
        </el-form-item>
      </el-form>
...
export default Vue.extend({
  ...
  created () {
    this.initUploader()
    getLessonById(this.$route.query.lessonId).then(({ data }) => {
      this.courseInfo = data.data
    }).catch(err => {
      this.$message.error('获取课程信息失败：' + err)
    })
  },
  methods: {
    authUpload () {
